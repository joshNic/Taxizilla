...
export function bookCar() {
  return (dispatch, store) => {
    ...
    const vr = nearByDrivers.length;
    console.log("__---'___", nearByDrivers);
    const nearByDriver = nearByDrivers[Math.floor(Math.random() * vr)];
    const payload = {
      data: {
        userName: 'Mugisha',
        pickUp: {
...
export function getNearByDrivers() {
  return (dispatch, store) => {
    request
      .get('http://localhost:3000/api/driverslocationSocket')
      .query({
        latitude: 3.145909,
